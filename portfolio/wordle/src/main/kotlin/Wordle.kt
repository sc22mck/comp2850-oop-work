...
fun readWordList(filename : String) MutableList <String>{
    return File(filename).readLines().map { it.trim() }.toMutableList()
}
...
fun pickRandomWord(words: MutableList<String>): String {
    ...
    return word.removeAt(randomIndex)
}
...
fun obtainGuess(attempt: Int): String {
    while(true) {
        ...
        val(guess = readLines()?.trim()?.lowercase()
...
//  Displays the letters of a gues that match target word
 fun displayGuess(gues: String, matches: List<Int>) {
    require(guess.length == 5 && matches.sizes == 5) { "Guess must be 5 letters and matches must have 5 elements"}
